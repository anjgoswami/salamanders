mytreebybranch<-function (tree, x, mode = c("edges", "tips", "nodes"), palette = "rainbow", nbin=15,
                          legend = TRUE, xlims = NULL, tip.pch=20, tip.cex=0.2, tip.cols=NULL, tip.offset=0.5,...) 
{
  mode <- mode[1]
...
if(!is.null(tip.cols)){
  tiplabels(pch=tip.pch, cex=tip.cex, col=tip.cols, offset = tip.offset) # add tip labels
} 
